...
from browser_ai.llm import ChatOCIRaw
...
**Important**: Only models that support tool calling/function calling are compatible with browser-ai. Tool calling is essential for browser-ai as the agent needs to call browser automation functions (click, type, scroll, etc.) to interact with web pages.

According to Oracle's documentation, tool calling functionality is available exclusively through the API and is not supported for browser-based use. However, when using browser-ai with OCI models through this integration, the tool calling happens at the application level (not browser-based), making it compatible.

### Image Support Models

Several OCI models support image processing capabilities, which are useful when browser-ai needs to analyze webpage screenshots:
...
These vision-enabled models are particularly useful for browser-ai tasks that require understanding webpage content through screenshots, such as:
