...
Browser Actor is a web automation library built on CDP (Chrome DevTools Protocol) that provides low-level browser automation capabilities within the browser-ai ecosystem.

## Usage

### Integrated with Browser (Recommended)
```python
from browser_ai import Browser  # Alias for BrowserSession
...
```python
from browser_ai.actor import Page, Element, Mouse
...
**This is browser-ai actor, NOT Playwright or Selenium.** Only use the methods documented above.
