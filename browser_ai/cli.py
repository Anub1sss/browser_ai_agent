...
if 'init' in sys.argv:
	from browser_ai.init_cmd import INIT_TEMPLATES
	from browser_ai.init_cmd import main as init_main
...
from browser_ai import Agent, Controller
from browser_ai.agent.views import AgentSettings
from browser_ai.browser import BrowserProfile, BrowserSession
from browser_ai.logging_config import addLoggingLevel
from browser_ai.telemetry import CLITelemetryEvent, ProductTelemetry
from browser_ai.utils import get_browser_use_version
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('browser_ai.'):
		...
		for logger_name in ['browser_ai.Agent', 'browser_ai.controller', 'browser_ai.agent', 'browser_ai.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'browser_ai.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('browser_ai.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('browser_ai.app')
...
async def textual_interface(config: dict[str, Any]):
	...
	logger = logging.getLogger('browser_ai.startup')
	...
	try:
		...
		try:
			from browser_ai.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from browser_ai.sync.auth import DeviceAuthClient
	...
	try:
		...
		from browser_ai.sync.service import CloudSync
		...
		if task_id and sync_service:
			try:
				from browser_ai.agent.cloud_events import UpdateAgentTaskEvent
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from browser_ai.mcp.server import main as mcp_main
